...
- **Z-order.** SVG render order = z-order (later elements appear on top). Arrange shape
  groups so that backgrounds come first, then panels/cards, then text, then accent elements on top.
  Connectors/lines that cross other shapes should be placed after the shapes they cross.
